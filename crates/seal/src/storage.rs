...
#[host(seal0)]
pub fn seal_get_storage(key_ptr: u32, out_ptr: u32, out_len_ptr: u32) -> Result<Value> {
...
#[host(seal0)]
pub fn seal_clear_storage(key_ptr: u32) -> Result<Value> {
    let mut key: StorageKey = [0; 32];
    sandbox.read_sandbox_memory_into_buf(key_ptr, &mut key)?;
...
#[host(seal0)]
pub fn seal_set_storage(key_ptr: u32, value_ptr: u32, value_len: u32) -> Result<Value> {
    let mut key: StorageKey = [0; 32];
    sandbox.read_sandbox_memory_into_buf(key_ptr, &mut key)?;
